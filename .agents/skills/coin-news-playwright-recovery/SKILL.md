...
6. Verify with `python -m py_compile`, `git diff --check`, and one real cycle with DingTalk disabled. Clearly distinguish static checks, real-browser validation, and intentionally untested retry branches.
7. Re-check the diff and preserve concurrent user edits such as environment-default changes.